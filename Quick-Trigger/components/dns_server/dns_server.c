...
static const char *TAG4 = "dns_redirect_server";
...
static int parse_dns_request(char *req, size_t req_len, char *dns_reply, size_t dns_reply_max_len)
{
    ...
    ESP_LOGD(TAG4, "DNS query with header id: 0x%X, flags: 0x%X, qd_count: %d",
    ...
    for (int i = 0; i < qd_count; i++) {
        ...
        if (name_end_ptr == NULL) {
            ESP_LOGE(TAG4, "Failed to parse DNS question: %s", cur_qd_ptr);
            return -1;
        }
        ...
        ESP_LOGD(TAG4, "Received type: %d | Class: %d | Question for: %s", qd_type, qd_class, name);
        ...
        if (qd_type == QD_TYPE_A) {
            ...
            ESP_LOGD(TAG4, "Answer with PTR offset: 0x%X and IP 0x%X", ntohs(answer->ptr_offset), ip_info.ip.addr);
...
void dns_server_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        if (sock < 0) {
            ESP_LOGE(TAG4, "Unable to create socket: errno %d", errno);
            break;
        }
        ESP_LOGI(TAG4, "Socket created");

        int err = bind(sock, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
        if (err < 0) {
            ESP_LOGE(TAG4, "Socket unable to bind: errno %d", errno);
        }
        ESP_LOGI(TAG4, "Socket bound, port %d", DNS_PORT);

        while (1) {
            ESP_LOGI(TAG4, "Waiting for data");
            ...
            if (len < 0) {
                ESP_LOGE(TAG4, "recvfrom failed: errno %d", errno);
                close(sock);
                break;
            }
            // Data received
                ...
                ESP_LOGI(TAG4, "Received %d bytes from %s | DNS reply with len: %d", len, addr_str, reply_len);
                if (reply_len <= 0) {
                    ESP_LOGE(TAG4, "Failed to prepare a DNS reply");
                } else {
                    int err = sendto(sock, reply, reply_len, 0, (struct sockaddr *)&source_addr, sizeof(source_addr));
                    if (err < 0) {
                        ESP_LOGE(TAG4, "Error occurred during sending: errno %d", errno);
                        break;
                    }
                }
            }
        ...
        if (sock != -1) {
            ESP_LOGE(TAG4, "Shutting down socket");
            shutdown(sock, 0);
            close(sock);
        }
    }
